...
#define REPLAY_REMEMBER_MASK 0x0A /*1024*/              // Mask for wrapping packet memory index
...
typedef struct ReplayWire {
    ...
    // Advertisement payload is range map, repeat(bitmap, priority, uint16[16] packet hashes), aggregate mask, throttled clients
    // Request payload is uint16 bitmap of 16-bit address chunks, then one uint16 bitmap of requested packets per flagged chunk
    /**
...
class ReplayModule : public SinglePortModule, private concurrency::NotifiedWorkerThread
{
  public:
    ...
    {
        LOG_INFO("Replay: self_heap=%u", sizeof(*this));
    }
    void adopt(meshtastic_MeshPacket *p);
  ...
  private:
    ...
    void handleAvailabilityAdvertisement(ReplayWire *wire, uint16_t *payload, int payload_words, ReplayServerInfo *server);
    void handleExpiredAdvertisement(ReplayWire *wire, uint16_t *payload, ReplayServerInfo *server);
